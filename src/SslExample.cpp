...
namespace nds
{
	...
	namespace detail
	{
		...
		std::error_condition SslErrorCategory::default_error_condition(int error) const noexcept
		{
		...
		bool SslErrorCategory::equivalent(const std::error_code& error_code, int error_value) const noexcept
		{
			return *this == error_code.category() &&
				static_cast<int>(default_error_condition(error_code.value()).value()) == error_value;
		}

		bool SslErrorCategory::equivalent(int error, const std::error_condition& condition) const noexcept
		{
	...
	SslExample::~SslExample()
	{
		cleanup();
	}

	void SslExample::cleanup() {
